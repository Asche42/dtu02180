...
class Input
  def self.read(link_names)
    loop do
      ...
      if street_a == street_b || !link_names.keys.include?(street_a) || !link_names.keys.include?(street_b)
...
origin, _kb = Heureka::Pathfinding.parse_inference_file(open('clauses.txt').read)
