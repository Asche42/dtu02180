...
module Input
  def self.read(link_names) # rubocop:disable Metrics/MethodLength
    loop do
      ...
      if street_a == street_b || !link_names.keys.include?(street_a) \
                              || !link_names.keys.include?(street_b)
        STDERR.puts 'The input streets given are invalid.'
        next # Let's give the user another shot...
      end

      input = (Set.new(link_names[street_a]) \
               & Set.new(link_names[street_b])).to_a.first
...
_dataset, link_names = Heureka::Pathfinding.parse(open('manhattan.txt').read)
...
puts "The chosen streets are: #{street_a} and #{street_b}, " \
     "the origin point is at: (#{origin.x}, #{origin.y})."
...
puts "The chosen streets are: #{street_a} and #{street_b}, " \
     "the destination point is at: (#{destination.x}, #{destination.y})."

path = Heureka::Pathfinding::Astar.process(origin, destination)
